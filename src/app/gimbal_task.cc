...
void Gimbal::StateUpdate()
{
    ...
    else
    {
        ...
        case RcSwitchState::kUp:
            ...
            {
                StateMachine_ =
                    GM_MATCH; // 左拨杆拨到下侧，进入比赛模式，此时全部系统都上电工作
            }
            break;
        ...
        case RcSwitchState::kMid:
            ...
            {
                StateMachine_ =
                    GM_TEST; // 左拨杆拨到下侧，进入测试模式，此时除发射系统外都工作
            }
            break;
...
void Gimbal::ChassisStateUpdate()
{
    chassis_x_rc_ =
        Constrain(remote->right_x() / 660.0f - (f32)remote->key(RcKey::kA) + (f32)remote->key(RcKey::kD) -
                      (f32)(tcremote->data().keyboard_key >> 2 & 0x01) + (f32)(tcremote->data().keyboard_key >> 3 & 0x01),
                  -1.0f, 1.0f);
    chassis_y_rc_ =
        Constrain(remote->right_y() / 660.0f + (f32)remote->key(RcKey::kW) - (f32)remote->key(RcKey::kS) +
                      (f32)(tcremote->data().keyboard_key >> 0 & 0x01) - (f32)(tcremote->data().keyboard_key >> 1 & 0x01),
                  -1.0f, 1.0f);
    ChassisRequestStatePacket_.UiChange =
        (remote->key(RcKey::kR) == 1 || (tcremote->data().keyboard_key >> 8 & 0x01) == 1) ? 1 : 0; // Ui是否开启
    ...
    if (remote->key(RcKey::kCtrl) == 1 || tcremote->data().keyboard_key >> 5 & 0x01)
    {
        if (remote->key(RcKey::kV) == 1 || tcremote->data().keyboard_key >> 14 & 0x01)
        {
            aim_speed_change_ = -1;
        }
        else if (aim_speed_change_ == -1)
        {
            ...
            if (ChassisRequestStatePacket_.AimSpeedChange < -10)
                ChassisRequestStatePacket_.AimSpeedChange = -10;
            aim_speed_change_ = 0;
        }
        if (remote->key(RcKey::kB) == 1 || tcremote->data().keyboard_key >> 15 & 0x01)
        {
            aim_speed_change_ = 1;
        }
        else if (aim_speed_change_ == 1)
    ...
    case StateMachineType::GM_TEST: // 测试模式下，底盘正常运行，有陀螺
        ChassisRequestStatePacket_.ChassisMoveXRequest = (i8)(chassis_x_rc_ * 100.0f);
        ChassisRequestStatePacket_.ChassisMoveYRequest = (i8)(chassis_y_rc_ * 100.0f);
        ...
        if (remote->dial() == 660 || remote->key(RcKey::kShift) == 1 || (tcremote->data().keyboard_key >> 4 & 0x01) == 1)
        {
        ...
        if (remote->key(RcKey::kC) == 1 || (tcremote->data().keyboard_key >> 13 & 0x01) == 1)
        {
    ...
    case GM_SINGLE_WHEEL: // 单轮模式下，底盘无陀螺
        ...
        {
            if (i != 3)
                ChassisRequestStatePacket_.ChassisStateRequest &= ~(u8)(1 << i); // 清除第 2、3、5、6、7 位
        }
        break;
    ...
    case GM_MATCH: // 比赛模式下，底盘正常运行，遥控器拨盘不控制小陀螺
        ...
        ChassisRequestStatePacket_.ChassisStateRequest &= ~(u8)(1 << 2); // 清除第 3 位
        if (remote->key(RcKey::kShift) == 1 || (tcremote->data().keyboard_key >> 4 & 0x01) == 1)
        {
        ...
        if (remote->key(RcKey::kZ) == 1 || (tcremote->data().keyboard_key >> 11 & 0x01) == 1)
        {
    ...
    case GM_HIGH_SPEED:
        ...
        if (remote->dial() == 660 || (i16)remote->key(RcKey::kShift) == 1 ||
            (i16)(tcremote->data().keyboard_key >> 4 & 0x01) == 1)
        {
...
void Gimbal::GimbalEnableUpdate()
{
    ...
    yaw_motor_.SetCurrent(
        static_cast<i16>(yaw_pid_speed_.value() + kyaw_current_ * yaw_motor_.rpm()));
...
void Gimbal::GimbalMatchUpdate()
{
    ...
    if ((remote->key(RcKey::kF) == 1 || (tcremote->data().keyboard_key >> 9 & 0x01) == 1) && XFstate_ == 0)
    {
        DFflag_ = 1;
    }
    else if (DFflag_ == 1)
    ...
    if ((remote->key(RcKey::kG) == 1 || (tcremote->data().keyboard_key >> 10 & 0x01) == 1) && DFstate_ == 0)
    {
        XFflag_ = 1;
    }
    else if (XFflag_ == 1)
    ...
    if (gimbal_imu_frame.mode == 0x01 && ((aimbot_frame.AimbotState >> 0) & 0x01) == 1 && aimbot_frame.AimbotTarget != 0x20 &&
        (remote->mouse_button_right() == 1 || tcremote->data().mouse_button_right == 1))
    {
        AimbotDateUpdate();
        AimPIDUpdate();
        yaw_motor_.SetCurrent(
            static_cast<i16>(aimbot_yaw_pid_speed_.value() + kyaw_current_ * yaw_motor_.rpm()));
    ...
    else if (gimbal_imu_frame.mode == 0x04 && ((aimbot_frame.AimbotState >> 0) & 0x01) == 1)
    {
        ...
        BuffPIDUpdate();
        yaw_motor_.SetCurrent(
            static_cast<i16>(buff_yaw_pid_speed_.value() + kyaw_current_ * yaw_motor_.rpm()));
...
{
    ammo_pid_left_.Update(-6300 - ChassisRequestStatePacket_.AimSpeedChange * 20, ammo_left_.rpm());
    ammo_pid_right_.Update(6300 + ChassisRequestStatePacket_.AimSpeedChange * 20, ammo_right_.rpm());
...
{
    if ((remote->dial() >= 650 || remote->mouse_button_left() == 1 || tcremote->data().mouse_button_left == 1) &&
        (remote->mouse_button_right() == 1 || tcremote->data().mouse_button_right == 1) &&
        ((aimbot_frame.AimbotState >> 1) & 0x01) == 1)
    {
        ammo_flag_rc_ = 1;
    }
    else if (remote->dial() >= 650 || remote->mouse_button_left() == 1 || tcremote->data().mouse_button_left == 1)
    {
        ammo_flag_rc_ = 1;
    }
    ...
    if ((rotor_target_position_ - rotor_position_) < 2000.0f && ammo_flag_rc_ == 2)
    {
        ammo_flag_rc_ = 0;
    }
...
void Gimbal::RotorMatchUpdate()
{
    if (gimbal_imu_frame.mode == 0x01)
    {
        ...
        else if (remote->mouse_button_right() == 0 && tcremote->data().mouse_button_right == 0 &&
                 (remote->dial() >= 650 || remote->mouse_button_left() == 1 || tcremote->data().mouse_button_left == 1))
        {
            ammo_flag_rc_ = 1;
        }
        else if (remote->dial() <= -650)
    ...
    {
        if (remote->mouse_button_left() == 1 || tcremote->data().mouse_button_left == 1)
        {
            shoot_flag_ = 1;
        }
...
{ // 没有单发模式，即时射击，无倒计时
    ...
    if ((ammo_flag_rc_ == 1) &&
        (heat_limit_ - heat_real_ - ((heat_delaytime_) / 2) >= 20))
    {
        if (heat_limit_ < 100)
        {
            rotor_pid_speed_.Update(2000, rotor_motor_.rpm());
            rotor_motor_.SetCurrent(rotor_pid_speed_.value());
        }
        else
        {
            rotor_pid_speed_.Update(2500, rotor_motor_.rpm());
            rotor_motor_.SetCurrent(rotor_pid_speed_.value());
        }
    }
    else if (ammo_flag_rc_ == 2)
    ...
    {
        rotor_pid_speed_.Update(-1000, rotor_motor_.rpm());
        rotor_motor_.SetCurrent(rotor_pid_speed_.value());
    }
    else
    {
...
{
    gimbal_yaw_rc_ -=
        Map(remote->left_x() + 10 * remote->mouse_x() + tcremote->data().mouse_x * 10, -660, 660, -0.3f, 0.3f);
    gimbal_pitch_rc_ -=
        Map(remote->left_y() + 10 * remote->mouse_y() + tcremote->data().mouse_y * 10, -660, 660, -0.2f, 0.2f);
}
...
void Gimbal::MovePIDUpdate()
{
    ...
    gimbal_pitch_rc_ = Constrain(gimbal_pitch_rc_, -35.0f, 30.0f); // pitch轴限位
    yaw_pid_position_.Update(gimbal_yaw_rc_, INS.yaw); // pid更新
    yaw_pid_speed_.Update(yaw_pid_position_.value() + kyaw_speed_ * yaw_motor_.rpm(),
                          yaw_motor_.rpm());
...
void Gimbal::AimPIDUpdate()
{
    ...
    aimbot_yaw_pid_speed_.Update(aimbot_yaw_pid_position_.value() + kyaw_speed_ * yaw_motor_.rpm(),
                                 yaw_motor_.rpm());
...
void Gimbal::BuffPIDUpdate()
{
    ...
    buff_yaw_pid_speed_.Update(buff_yaw_pid_position_.value() + kyaw_speed_ * yaw_motor_.rpm(),
                               yaw_motor_.rpm());
...
{
    if ((last_rotor_position_ - rotor_motor_.encoder()) >= 5000.f)
    { // 过圈检测
        rotor_circle_flag_++;
    }
